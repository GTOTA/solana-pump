        ...
        try {
            ...
            if (successfulResults.length > 0) {
                successfulResults.map((result) => {
                    const bundle_id = result.data.result
                    console.log("sent to frankfurt, bundle result: ", bundle_id)
                    if (result.data.result.value && result.data.result.value.length > 0) {
                        const landingSlot = result.data.result.value[0].slot;
                        console.log(`Slot : ${landingSlot}`);
                    } else {
                        console.log('Landing failed');
                    }
                    return { confirmed: true, jitoTxsignature };
                })
                wait(1000)
            ...
            if (error instanceof AxiosError) {
                console.error({ error: error.response?.data }, 'Failed to execute jito transaction');
            }
