...
export function parseNewPoolMessage(text: string): CryptoAlert {
  // Split the text into lines
  const lines = text.split('\n');
  const result: CryptoAlert = {};

  // Regular expressions
  const emojiRegex = /[\u{1F300}-\u{1F9FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}]|[\u{1F100}-\u{1F1FF}]|[\u{1F200}-\u{1F2FF}]|[\u{1F300}-\u{1F3FF}]|[\u{1F400}-\u{1F4FF}]|[\u{1F500}-\u{1F5FF}]|[\u{1F600}-\u{1F64F}]|[\u{1F680}-\u{1F6FF}]|[\u{1F700}-\u{1F77F}]|[\u{1F780}-\u{1F7FF}]|[\u{1F800}-\u{1F8FF}]|[\u{1F900}-\u{1F9FF}]|[\u{1FA00}-\u{1FA6F}]|[\u{1FA70}-\u{1FAFF}]/gu;
  const chineseRegex = /[\u4e00-\u9fff]/g;
  ...
  lines.forEach(line => {
    ...
    if(!namefound) {
      line = line.replace(/\*\*/g, '');
      const items = line.split('(').map(key => key.trim());
      result['symbol'] = items[0].substring(0,items[0].length)
      result['name'] = items[1].substring(0,items[1].length-1)
      namefound = true
      return
    }
    ...
    // 4. Remove emojis
    key = key.replace(/^[-]/, '');
    // 5. Trim again and remove any remaining whitespace
    key = key.toLowerCase().trim().replace(' ','')
    ...
    if(key == 'liq') {
      ...
      value = value.substring(1,index-1).trim();
    }
    if(key == 'price') {
      const index = value.indexOf(' ')
      value = convertNumber(value.substring(1,index).trim());
    } 
    if(key.includes('top10')){
      key = key.substring(0,5)
      value = value.split(' ').map(key => key.trim())[0]
    }
    if (key == 'renounced' && emojiMap[value] != undefined) {
      value = emojiMap[value] + ''
    }
